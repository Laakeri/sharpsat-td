...
public:
  ...
  ComponentArchetype(StackLevel<T_num> &stack_level, Component &super_comp) :
      p_super_comp_(&super_comp), p_stack_level_(&stack_level) {
  }

  void reInitialize(StackLevel<T_num> &stack_level, Component &super_comp) {
...
//    }
  ...
  inline void createComponents(Component &ret_comp, CacheableComponent<T_num> ret_cache_comp,
      unsigned stack_size) {
...
private:
  ...
  StackLevel<T_num> *p_stack_level_;
...
template <class T_num>
CA_SearchState *ComponentArchetype<T_num>::seen_ = nullptr;

template <class T_num>
unsigned ComponentArchetype<T_num>::seen_byte_size_ = 0;
